...
func TestUpgradePipelineStep(t *testing.T) {
	...
	stepAvn, err := avnGen.UpgradePipelineStepGet(ctx, organizationID, step.Status.ID)
	require.NoError(t, err)
	...
	require.Eventually(t, func() bool {
		out, err := avnGen.UpgradePipelineStepGet(ctx, organizationID, updatedStep.Status.ID)
		if err != nil {
			return false
		}
	...
	require.NoError(t, s.Delete(updatedStep, func() error {
		_, err := avnGen.UpgradePipelineStepGet(ctx, organizationID, updatedStep.Status.ID)
		return err
	}))
	require.Eventually(t, func() bool {
		out, err := avnGen.UpgradePipelineStepList(ctx, organizationID)
		if err != nil {
			return false
		}
	...
	adopted, err := avnGen.UpgradePipelineStepCreate(ctx, organizationID, &upgradepipeline.UpgradePipelineStepCreateIn{
	...
	defer func() {
		...
		_ = avnGen.UpgradePipelineStepDelete(cleanupCtx, organizationID, adoptedStepID)
	}()
	...
	require.Eventually(t, func() bool {
		out, err := avnGen.UpgradePipelineStepGet(ctx, organizationID, updatedAdoptedStep.Status.ID)
		if err != nil {
			return false
		}
	...
	require.NoError(t, s.Delete(updatedAdoptedStep, func() error {
		_, err := avnGen.UpgradePipelineStepGet(ctx, organizationID, updatedAdoptedStep.Status.ID)
		return err
	}))
}
