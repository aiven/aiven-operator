...
- [`migrationSecretSource`](#spec.migrationSecretSource-property){: name='spec.migrationSecretSource-property'} (object). Reference to a Secret containing migration credentials.
    ...
    at reconcile time so malformed credentials surface clearly instead of being silently altered. See below for [nested schema](#spec.migrationSecretSource).
...
- [`enable_ha_replica_dns`](#spec.userConfig.enable_ha_replica_dns-property){: name='spec.userConfig.enable_ha_replica_dns-property'} (boolean). Creates a dedicated read-only DNS that automatically falls back to the primary if standby nodes are unavailable. It switches back when a standby recovers.
...
- [`node_count`](#spec.userConfig.node_count-property){: name='spec.userConfig.node_count-property'} (integer, Minimum: 1, Maximum: 100). Number of nodes for the service.
...
- [`pg_stat_plans_enable`](#spec.userConfig.pg_stat_plans_enable-property){: name='spec.userConfig.pg_stat_plans_enable-property'} (boolean). Enable the pg_stat_plans extension. Changing this parameter causes a service restart. Tracks execution plans for SQL queries.
- [`pg_version`](#spec.userConfig.pg_version-property){: name='spec.userConfig.pg_version-property'} (string). Available versions: `14`, `15`, `16`, `17`, `18`. Newer versions may also be available.
...
- [`io_combine_limit`](#spec.userConfig.pg.io_combine_limit-property){: name='spec.userConfig.pg.io_combine_limit-property'} (integer, Minimum: 1, Maximum: 32). EXPERIMENTAL: Controls the largest I/O size in operations that combine I/O in 8kB units. Version 17 and up only.
- [`io_max_combine_limit`](#spec.userConfig.pg.io_max_combine_limit-property){: name='spec.userConfig.pg.io_max_combine_limit-property'} (integer, Minimum: 1, Maximum: 128). EXPERIMENTAL: Controls the largest I/O size in operations that combine I/O in 8kB units, and silently limits the user-settable parameter io_combine_limit. Version 18 and up only. Changing this parameter causes a service restart.
- [`io_max_concurrency`](#spec.userConfig.pg.io_max_concurrency-property){: name='spec.userConfig.pg.io_max_concurrency-property'} (integer, Minimum: -1, Maximum: 1024). EXPERIMENTAL: Controls the maximum number of I/O operations that one process can execute simultaneously. Version 18 and up only. Changing this parameter causes a service restart.
- [`io_method`](#spec.userConfig.pg.io_method-property){: name='spec.userConfig.pg.io_method-property'} (string, Enum: `io_uring`, `sync`, `worker`). EXPERIMENTAL: Controls the maximum number of I/O operations that one process can execute simultaneously. Version 18 and up only. Changing this parameter causes a service restart.
- [`io_workers`](#spec.userConfig.pg.io_workers-property){: name='spec.userConfig.pg.io_workers-property'} (integer, Minimum: 1, Maximum: 32). EXPERIMENTAL: Number of IO worker processes, for io_method=worker. Version 18 and up only.
...
- [`pg_stat_monitor.pgsm_enable_query_plan`](#spec.userConfig.pg.pg_stat_monitor.pgsm_enable_query_plan-property){: name='spec.userConfig.pg.pg_stat_monitor.pgsm_enable_query_plan-property'} (boolean). Enables or disables query plan monitoring. Only available for PostgreSQL 13+.
...
- [`pg_stat_plans.track`](#spec.userConfig.pg.pg_stat_plans.track-property){: name='spec.userConfig.pg.pg_stat_plans.track-property'} (string, Enum: `all`, `none`, `top`). Controls which statements' plans are tracked. Specify top to track top-level statements (those issued directly by clients), all to also track nested statements (such as statements invoked within functions), or none to disable plan tracking. The default is `top`.
...
- [`synchronous_commit`](#spec.userConfig.pg.synchronous_commit-property){: name='spec.userConfig.pg.synchronous_commit-property'} (string, Enum: `local`, `off`, `on`, `remote_apply`, `remote_write`). Sets the current transaction's synchronization level. The default is `off`. This setting takes precedence over `synchronous_replication`.
...
- [`server_connect_timeout`](#spec.userConfig.pgbouncer.server_connect_timeout-property){: name='spec.userConfig.pgbouncer.server_connect_timeout-property'} (number, Minimum: 1, Maximum: 86400). If connection and login don’t finish in this amount of time, the connection will be closed. [seconds].
...
- [`server_login_retry`](#spec.userConfig.pgbouncer.server_login_retry-property){: name='spec.userConfig.pgbouncer.server_login_retry-property'} (number, Minimum: 1, Maximum: 86400). If login to the server failed, because of failure to connect or from authentication, the pooler waits this much before retrying to connect. During the waiting interval, new clients trying to connect to the failing server will get an error immediately without another connection attempt. [seconds].
...
### switchover_windows {: #spec.userConfig.switchover_windows }

_Appears on [`spec.userConfig`](#spec.userConfig)._

**Required**

- [`dow`](#spec.userConfig.switchover_windows.dow-property){: name='spec.userConfig.switchover_windows.dow-property'} (string, Enum: `friday`, `monday`, `saturday`, `sunday`, `thursday`, `tuesday`, `wednesday`).
- [`end_time`](#spec.userConfig.switchover_windows.end_time-property){: name='spec.userConfig.switchover_windows.end_time-property'} (string, MinLength: 8, MaxLength: 8).
- [`start_time`](#spec.userConfig.switchover_windows.start_time-property){: name='spec.userConfig.switchover_windows.start_time-property'} (string, MinLength: 8, MaxLength: 8).
...
- [`max_background_workers`](#spec.userConfig.timescaledb.max_background_workers-property){: name='spec.userConfig.timescaledb.max_background_workers-property'} (integer, Minimum: 1, Maximum: 4096). The number of background workers for timescaledb operations. You should configure this setting to the sum of your number of databases and the total number of concurrent background workers you want running at any given point in time. Changing this parameter causes a service restart.
