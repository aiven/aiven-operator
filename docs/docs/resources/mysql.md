...
- [`migrationSecretSource`](#spec.migrationSecretSource-property){: name='spec.migrationSecretSource-property'} (object). Reference to a Secret containing migration credentials.
    ...
    Values must not contain leading or trailing whitespace; such values are rejected. See below for [nested schema](#spec.migrationSecretSource).
...
- [`deleteAfterMigration`](#spec.migrationSecretSource.deleteAfterMigration-property){: name='spec.migrationSecretSource.deleteAfterMigration-property'} (boolean, Default value: `false`). When true, the operator deletes the referenced Kubernetes Secret after
    ...
    The field remains mutable post-migration: flipping false→true on a Done
    resource deletes the Secret on the next reconcile.
    Flipping true→false after deletion is a no-op.
...
- [`binlog_retention_period`](#spec.userConfig.binlog_retention_period-property){: name='spec.userConfig.binlog_retention_period-property'} (integer, Minimum: 600, Maximum: 9007199254740991). 
    !!! Warning
...
- [`mysql_version`](#spec.userConfig.mysql_version-property){: name='spec.userConfig.mysql_version-property'} (string). Available versions: `8`, `8.4`. Newer versions may also be available.
...
- [`dump_tool`](#spec.userConfig.migration.dump_tool-property){: name='spec.userConfig.migration.dump_tool-property'} (string, Enum: `mydumper`, `mysqldump`). Experimental! Tool to use for database dump and restore during migration. Default: mysqldump.
...
- [`reestablish_replication`](#spec.userConfig.migration.reestablish_replication-property){: name='spec.userConfig.migration.reestablish_replication-property'} (boolean). Skip dump-restore part and start replication.
...
- [`automatic_sp_privileges`](#spec.userConfig.mysql.automatic_sp_privileges-property){: name='spec.userConfig.mysql.automatic_sp_privileges-property'} (boolean). When enabled, the server automatically grants the EXECUTE and ALTER ROUTINE privileges to the creator of a stored routine and drops them when the routine is dropped.
...
- [`div_precision_increment`](#spec.userConfig.mysql.div_precision_increment-property){: name='spec.userConfig.mysql.div_precision_increment-property'} (integer, Minimum: 0, Maximum: 30). Number of digits by which to increase the scale of the result of division operations performed with the / operator. Default is 4.
- [`end_markers_in_json`](#spec.userConfig.mysql.end_markers_in_json-property){: name='spec.userConfig.mysql.end_markers_in_json-property'} (boolean). Whether optimizer JSON output such as EXPLAIN FORMAT=JSON adds end markers that repeat a structure's key near its closing bracket, making large JSON structures easier to read.
- [`eq_range_index_dive_limit`](#spec.userConfig.mysql.eq_range_index_dive_limit-property){: name='spec.userConfig.mysql.eq_range_index_dive_limit-property'} (integer, Minimum: 0, Maximum: 4294967295). The number of equality ranges in a query at or above which the optimizer switches from index dives to index statistics when estimating the number of qualifying rows. 0 means always use index dives. Default is 200.
...
- [`innodb_adaptive_hash_index`](#spec.userConfig.mysql.innodb_adaptive_hash_index-property){: name='spec.userConfig.mysql.innodb_adaptive_hash_index-property'} (boolean). Whether InnoDB adaptive hash indexing is enabled. The optimal setting is workload-dependent: it speeds up lookups for some workloads but its internal latch can become a contention point under high concurrency, in which case disabling it can improve throughput.
...
- [`innodb_ft_enable_stopword`](#spec.userConfig.mysql.innodb_ft_enable_stopword-property){: name='spec.userConfig.mysql.innodb_ft_enable_stopword-property'} (boolean). Whether stopword processing is applied when creating or rebuilding an InnoDB FULLTEXT index. Enabled by default.
- [`innodb_ft_max_token_size`](#spec.userConfig.mysql.innodb_ft_max_token_size-property){: name='spec.userConfig.mysql.innodb_ft_max_token_size-property'} (integer, Minimum: 10, Maximum: 84). Maximum length of words that are stored in an InnoDB FULLTEXT index. Changing this parameter will lead to a restart of the MySQL service.
...
- [`innodb_ft_num_word_optimize`](#spec.userConfig.mysql.innodb_ft_num_word_optimize-property){: name='spec.userConfig.mysql.innodb_ft_num_word_optimize-property'} (integer, Minimum: 1000, Maximum: 10000). Number of words processed during each OPTIMIZE TABLE operation on an InnoDB FULLTEXT index. Default is 2000.
- [`innodb_ft_result_cache_limit`](#spec.userConfig.mysql.innodb_ft_result_cache_limit-property){: name='spec.userConfig.mysql.innodb_ft_result_cache_limit-property'} (integer, Minimum: 1000000, Maximum: 4294967295). Maximum memory in bytes used per query for the InnoDB FULLTEXT search query result cache. Aiven sizes this automatically based on the service plan's memory; setting a value overrides the calculated default.
...
- [`innodb_ft_user_stopword_table`](#spec.userConfig.mysql.innodb_ft_user_stopword_table-property){: name='spec.userConfig.mysql.innodb_ft_user_stopword_table-property'} (string, Pattern: `^.+/.+$`, MaxLength: 1024). This option is used to specify your own InnoDB FULLTEXT index stopword list for specific InnoDB tables.
- [`innodb_io_capacity`](#spec.userConfig.mysql.innodb_io_capacity-property){: name='spec.userConfig.mysql.innodb_io_capacity-property'} (integer, Minimum: 100, Maximum: 4294967295). The number of I/O operations per second (IOPS) available to InnoDB background tasks, such as flushing pages from the buffer pool and merging data from the change buffer. Set this to a value appropriate for the underlying storage; it must not exceed innodb_io_capacity_max.
- [`innodb_io_capacity_max`](#spec.userConfig.mysql.innodb_io_capacity_max-property){: name='spec.userConfig.mysql.innodb_io_capacity_max-property'} (integer, Minimum: 100, Maximum: 4294967295). The maximum number of I/O operations per second (IOPS) that InnoDB background tasks may perform when flushing falls behind. Defaults to twice innodb_io_capacity (minimum 2000). This must be greater than or equal to innodb_io_capacity.
...
- [`innodb_log_buffer_size`](#spec.userConfig.mysql.innodb_log_buffer_size-property){: name='spec.userConfig.mysql.innodb_log_buffer_size-property'} (integer, Minimum: 1048576, Maximum: 4294967296). The size in bytes of the buffer that InnoDB uses to write to the log files on disk. Requests above 15% of the RAM provided by your service plan are rejected, because a larger buffer leaves less memory for the buffer pool and client connections.
...
- [`innodb_optimize_fulltext_only`](#spec.userConfig.mysql.innodb_optimize_fulltext_only-property){: name='spec.userConfig.mysql.innodb_optimize_fulltext_only-property'} (boolean). When enabled, OPTIMIZE TABLE on InnoDB tables only updates the FULLTEXT index instead of rebuilding the table. Intended to be enabled temporarily during FULLTEXT index maintenance and disabled afterwards; while enabled, OPTIMIZE TABLE does not reclaim table space.
...
- [`lower_case_table_names`](#spec.userConfig.mysql.lower_case_table_names-property){: name='spec.userConfig.mysql.lower_case_table_names-property'} (integer, Enum: `0`, `1`, Immutable). Sets how table and database names are stored and compared. 0 = case-sensitive (default), 1 = names stored lowercase, comparisons are case-insensitive. This option can only be set when creating the service and cannot be changed later. See https://dev.mysql.com/doc/refman/8.0/en/identifier-case-sensitivity.html for details.
- [`max_allowed_packet`](#spec.userConfig.mysql.max_allowed_packet-property){: name='spec.userConfig.mysql.max_allowed_packet-property'} (integer, Minimum: 102400, Maximum: 1073741824). Size of the largest message in bytes that can be received by the server. Default is 67108864 (64M). Statements and rows larger than this are rejected with a packet too large error.
- [`max_connections`](#spec.userConfig.mysql.max_connections-property){: name='spec.userConfig.mysql.max_connections-property'} (integer, Minimum: 30, Maximum: 100000). The maximum permitted number of simultaneous client connections. Lower this to reserve memory for other work. The value cannot exceed the limit provided by your service plan. Upgrading the plan does not raise a value you have set explicitly, so increase it yourself after an upgrade.
- [`max_execution_time`](#spec.userConfig.mysql.max_execution_time-property){: name='spec.userConfig.mysql.max_execution_time-property'} (integer, Minimum: 0, Maximum: 4294967295). Execution timeout in milliseconds for read-only top-level SELECT statements. 0 (the default) means no timeout.
...
- [`max_seeks_for_key`](#spec.userConfig.mysql.max_seeks_for_key-property){: name='spec.userConfig.mysql.max_seeks_for_key-property'} (integer, Minimum: 1). Limit on the assumed maximum number of index seeks when looking up rows based on a key. Lowering this value causes the optimizer to prefer index lookups over table scans.
- [`max_user_connections`](#spec.userConfig.mysql.max_user_connections-property){: name='spec.userConfig.mysql.max_user_connections-property'} (integer, Minimum: 0, Maximum: 99990). The maximum number of simultaneous connections permitted to any single user account. 0, the default, means no per-account limit. Any other value must be at least 10 below max_connections, so that monitoring and your own admin sessions can still connect when an application saturates its own limit. Aiven's replication and management connections are unaffected however low you set this.
...
- [`optimizer_prune_level`](#spec.userConfig.mysql.optimizer_prune_level-property){: name='spec.userConfig.mysql.optimizer_prune_level-property'} (integer, Minimum: 0, Maximum: 1). Controls the heuristics applied during query optimization to prune less-promising partial plans from the optimizer search space. 0 disables heuristics (exhaustive search); 1 prunes plans based on the number of rows retrieved.
- [`optimizer_search_depth`](#spec.userConfig.mysql.optimizer_search_depth-property){: name='spec.userConfig.mysql.optimizer_search_depth-property'} (integer, Minimum: 0, Maximum: 62). Maximum depth of search performed by the query optimizer when choosing a join order. Larger values produce better plans for joins over many tables but take longer to compile; 0 lets the optimizer choose the depth automatically.
- [`optimizer_switch`](#spec.userConfig.mysql.optimizer_switch-property){: name='spec.userConfig.mysql.optimizer_switch-property'} (string). Comma-separated list of optimizer flag assignments in the form flag=on|off|default, or the single value `default` to reset all flags. Flags not listed keep their current values. Controls query optimizer behaviors such as index merge, hash join and semijoin strategies.
- [`performance_schema_events_statements_history_size`](#spec.userConfig.mysql.performance_schema_events_statements_history_size-property){: name='spec.userConfig.mysql.performance_schema_events_statements_history_size-property'} (integer, Minimum: 0, Maximum: 1024). The number of rows per thread in the events_statements_history table. Changing this parameter will lead to a restart of the MySQL service.
- [`relay_log_space_limit`](#spec.userConfig.mysql.relay_log_space_limit-property){: name='spec.userConfig.mysql.relay_log_space_limit-property'} (integer, Minimum: 134217728). The maximum amount of space in bytes to use for all relay logs while replicating from an external migration source. When the limit is reached, the replication I/O thread stops fetching relay log events until the SQL thread has caught up. Raise this to give a large migration a bigger relay-log budget; ensure the service disk is sized accordingly. The setting applies only on the node replicating from the external source; standby nodes always use the Aiven-managed default (the smaller of 5 GiB and 30% of the service disk), which is also used when this option is left unset. Changing this parameter will lead to a restart of the MySQL service.
...
- [`sort_buffer_size`](#spec.userConfig.mysql.sort_buffer_size-property){: name='spec.userConfig.mysql.sort_buffer_size-property'} (integer, Minimum: 32768, Maximum: 1073741824). Sort buffer size in bytes for ORDER BY optimization. Default is 262144 (256K). Requests above 2% of the RAM provided by your service plan are rejected, because the buffer is allocated per session and its cost multiplies with the connection count.
...
- [`wait_timeout`](#spec.userConfig.mysql.wait_timeout-property){: name='spec.userConfig.mysql.wait_timeout-property'} (integer, Minimum: 1, Maximum: 2147483). The number of seconds the server waits for activity on a noninteractive connection before closing it. Requests to set this below 30 are rejected, because a shorter timeout closes your own idle connections between statements.
- [`windowing_use_high_precision`](#spec.userConfig.mysql.windowing_use_high_precision-property){: name='spec.userConfig.mysql.windowing_use_high_precision-property'} (boolean). Whether window functions are computed to high precision. Disabling this trades exactness for speed in window function evaluation.
