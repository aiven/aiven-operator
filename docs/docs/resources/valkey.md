...
- [`valkey_active_defrag_ignore_bytes`](#spec.userConfig.valkey_active_defrag_ignore_bytes-property){: name='spec.userConfig.valkey_active_defrag_ignore_bytes-property'} (integer, Minimum: 1048576, Maximum: 1073741824). Minimum amount of fragmentation waste, in bytes, before active defragmentation starts. Only takes effect when `valkey_activedefrag` is enabled.
- [`valkey_active_defrag_threshold_lower`](#spec.userConfig.valkey_active_defrag_threshold_lower-property){: name='spec.userConfig.valkey_active_defrag_threshold_lower-property'} (integer, Minimum: 1, Maximum: 100). Minimum percentage of fragmentation before active defragmentation starts. Only takes effect when `valkey_activedefrag` is enabled.
...
- [`valkey_activedefrag`](#spec.userConfig.valkey_activedefrag-property){: name='spec.userConfig.valkey_activedefrag-property'} (boolean). Enable active memory defragmentation. When enabled, Valkey relocates objects off sparsely-used memory pages to reduce fragmentation and return memory to the operating system. Defragmentation runs on the main thread and consumes CPU, so it may increase latency under load.
- [`valkey_io_threads`](#spec.userConfig.valkey_io_threads-property){: name='spec.userConfig.valkey_io_threads-property'} (integer, Minimum: 1, Maximum: 256). Set Valkey IO thread count. Changing this will cause a restart of the Valkey service.
