...
- [`karapace_version`](#spec.userConfig.karapace_version-property){: name='spec.userConfig.karapace_version-property'} (string, Pattern: `^[0-9]+\.[0-9]+\.[0-9]+$`). Pin a specific installed Karapace version on this service. Leave null/unset to auto-follow the newest installed version.
...
- [`sasl_oauthbearer_allowed_urls`](#spec.userConfig.sasl_oauthbearer_allowed_urls-property){: name='spec.userConfig.sasl_oauthbearer_allowed_urls-property'} (array of strings). List of allowed URLs for SASL OAUTHBEARER authentication. Only HTTPS URLs are allowed for security reasons.
...
- [`group_coordinator_rebalance_protocols`](#spec.userConfig.kafka.group_coordinator_rebalance_protocols-property){: name='spec.userConfig.kafka.group_coordinator_rebalance_protocols-property'} (string, Enum: `classic`, `classic,consumer`, `classic,consumer,share`, `classic,consumer,share,streams`, `classic,consumer,streams`, `classic,share`, `classic,streams`). The enabled consumer group rebalance protocols. Use consumer, classic, share, streams to enable Kafka share groups.
...
- [`group_share_delivery_count_limit`](#spec.userConfig.kafka.group_share_delivery_count_limit-property){: name='spec.userConfig.kafka.group_share_delivery_count_limit-property'} (integer, Minimum: 2, Maximum: 10). The maximum delivery attempts for a share-group record.
- [`group_share_heartbeat_interval_ms`](#spec.userConfig.kafka.group_share_heartbeat_interval_ms-property){: name='spec.userConfig.kafka.group_share_heartbeat_interval_ms-property'} (integer). The heartbeat interval used by share group members.
- [`group_share_max_groups`](#spec.userConfig.kafka.group_share_max_groups-property){: name='spec.userConfig.kafka.group_share_max_groups-property'} (integer). The maximum number of share groups allowed on the broker.
- [`group_share_max_heartbeat_interval_ms`](#spec.userConfig.kafka.group_share_max_heartbeat_interval_ms-property){: name='spec.userConfig.kafka.group_share_max_heartbeat_interval_ms-property'} (integer). The maximum heartbeat interval allowed for share group members.
- [`group_share_max_record_lock_duration_ms`](#spec.userConfig.kafka.group_share_max_record_lock_duration_ms-property){: name='spec.userConfig.kafka.group_share_max_record_lock_duration_ms-property'} (integer). The maximum record lock duration allowed for share groups.
- [`group_share_max_session_timeout_ms`](#spec.userConfig.kafka.group_share_max_session_timeout_ms-property){: name='spec.userConfig.kafka.group_share_max_session_timeout_ms-property'} (integer). The maximum session timeout allowed for share group members.
- [`group_share_max_size`](#spec.userConfig.kafka.group_share_max_size-property){: name='spec.userConfig.kafka.group_share_max_size-property'} (integer, Minimum: 10, Maximum: 1000). The maximum number of members allowed in a share group.
- [`group_share_min_heartbeat_interval_ms`](#spec.userConfig.kafka.group_share_min_heartbeat_interval_ms-property){: name='spec.userConfig.kafka.group_share_min_heartbeat_interval_ms-property'} (integer). The minimum heartbeat interval allowed for share group members.
- [`group_share_min_record_lock_duration_ms`](#spec.userConfig.kafka.group_share_min_record_lock_duration_ms-property){: name='spec.userConfig.kafka.group_share_min_record_lock_duration_ms-property'} (integer). The minimum record lock duration allowed for share groups.
- [`group_share_min_session_timeout_ms`](#spec.userConfig.kafka.group_share_min_session_timeout_ms-property){: name='spec.userConfig.kafka.group_share_min_session_timeout_ms-property'} (integer). The minimum session timeout allowed for share group members.
- [`group_share_partition_max_record_locks`](#spec.userConfig.kafka.group_share_partition_max_record_locks-property){: name='spec.userConfig.kafka.group_share_partition_max_record_locks-property'} (integer, Minimum: 100, Maximum: 10000). The maximum number of record locks allowed per share group partition.
- [`group_share_record_lock_duration_ms`](#spec.userConfig.kafka.group_share_record_lock_duration_ms-property){: name='spec.userConfig.kafka.group_share_record_lock_duration_ms-property'} (integer, Minimum: 1000, Maximum: 3600000). The duration for which a fetched share-group record is locked.
- [`group_share_session_timeout_ms`](#spec.userConfig.kafka.group_share_session_timeout_ms-property){: name='spec.userConfig.kafka.group_share_session_timeout_ms-property'} (integer). The timeout used to detect share group member failures.
...
- [`log_retention_hours`](#spec.userConfig.kafka.log_retention_hours-property){: name='spec.userConfig.kafka.log_retention_hours-property'} (integer, Minimum: -1, Maximum: 2147483647). The number of hours to keep a log file before deleting it. Use -1 for unlimited retention or 1 or higher. Setting 0 is invalid and prevents Kafka from starting. (Default: 168 hours, or 1 week).
...
- [`prefer_ipv6_address_enable`](#spec.userConfig.kafka_connect_config.prefer_ipv6_address_enable-property){: name='spec.userConfig.kafka_connect_config.prefer_ipv6_address_enable-property'} (boolean). When enabled, connectors will automatically resolve IPv6 addresses from external server names configured with dual-stack.
...
- [`server_pem`](#spec.userConfig.kafka_connect_secret_providers.vault.server_pem-property){: name='spec.userConfig.kafka_connect_secret_providers.vault.server_pem-property'} (string, MaxLength: 4096). PEM encoded certificate of the Vault server. Required if the vault server uses a self-signed certificate.
...
- [`sasl_oauthbearer_authentication_enabled`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_authentication_enabled-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_authentication_enabled-property'} (boolean). If enabled, the Schema Registry validates OAuth2/OIDC JWT bearer tokens on incoming requests. Requires the OIDC provider settings under the `kafka` configuration (`sasl_oauthbearer_jwks_endpoint_url` and related). Defaults to `false`.
- [`sasl_oauthbearer_authorization_enabled`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_authorization_enabled-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_authorization_enabled-property'} (boolean). If enabled, the Schema Registry enforces role-based authorization derived from the JWT roles claim. Requires `sasl_oauthbearer_authentication_enabled` to be enabled. Defaults to `false`.
- [`sasl_oauthbearer_method_roles`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_method_roles-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_method_roles-property'} (object). Mapping of HTTP methods to the list of roles allowed to perform them on the Schema Registry. Role names use the `karapace.` prefix, e.g. `karapace.schema:read`.
- [`sasl_oauthbearer_roles_claim_path`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_roles_claim_path-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_roles_claim_path-property'} (string, Pattern: `^[^\r\n]*\S[^\r\n]*$`, MaxLength: 128). JSON path used to extract the roles claim from the JWT for Schema Registry authorization. Defaults to `resource_access.karapace.roles`.
