...
- [`sasl_oauthbearer_authentication_enabled`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_authentication_enabled-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_authentication_enabled-property'} (boolean). If enabled, the Schema Registry validates OAuth 2.0/OIDC JWT bearer tokens. Requires `sasl_oauthbearer_jwks_endpoint_url`, `sasl_oauthbearer_expected_issuer`, and `sasl_oauthbearer_expected_audience` under `kafka`. Defaults to `false`.
- [`sasl_oauthbearer_authorization_enabled`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_authorization_enabled-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_authorization_enabled-property'} (boolean). If enabled, the Schema Registry enforces role-based authorization using the JWT roles claim. It also enables `sasl_oauthbearer_authentication_enabled` if it isn't already enabled. Authorization requires authentication. Defaults to `false`.
- [`sasl_oauthbearer_method_roles`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_method_roles-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_method_roles-property'} (string, MaxLength: 4096). Maps HTTP methods to allowed roles. Use a JSON object with `GET`, `POST`, `PUT`, and `DELETE` keys mapped to arrays of roles. Role names use the `karapace.` prefix. Example: `{"GET": ["karapace.schema:read"], "POST": [], "PUT": [], "DELETE": []}`.
- [`sasl_oauthbearer_roles_claim_path`](#spec.userConfig.schema_registry_config.sasl_oauthbearer_roles_claim_path-property){: name='spec.userConfig.schema_registry_config.sasl_oauthbearer_roles_claim_path-property'} (string, Pattern: `^[^\r\n]*\S[^\r\n]*$`, MaxLength: 128). The JSON path the Schema Registry uses to find the roles claim in the JWT. Defaults to `resource_access.karapace.roles`.
