...
//go:generate go run ./generators/userconfigs/... --services mysql,flink,grafana,pg,kafka,clickhouse,opensearch,kafka_connect,valkey
...
func main() {
	...
	flag.StringVar(&controllersSpec, "controllers", "*",
		"Kinds the operator reconciles: \"*\" for all, \"Kafka,KafkaTopic\" for an allow list, "+
			"\"*,-Flink\" to exclude kinds. Kinds left out get no controller, so their CRDs need not be installed.")
