...
func TestServiceUserRotationReconciler(t *testing.T) {
	...
	t.Run("Creates missing users and publishes initial credentials", func(t *testing.T) {
		...
		for _, user := range users {
			s.api.EXPECT().
				ServiceUserCreate(mock.Anything, s.rotation.Spec.Project, s.rotation.Spec.ServiceName, &service.ServiceUserCreateIn{Username: user.Username}).
				Return(new(service.ServiceUserCreateOut(user)), nil).Once()
		}
		s.api.EXPECT().
		...
		require.Contains(t, s.object(t).Finalizers, instanceDeletionFinalizer)
		auth := &corev1.Secret{}
		require.NoError(t, s.client.Get(t.Context(), crclient.ObjectKeyFromObject(s.auth), auth))
		require.Contains(t, auth.Finalizers, secretProtectionFinalizer)
	...
	t.Run("Recreates a missing active user without changing its published password", func(t *testing.T) {
		s := newRotationScenario(t)
		s.publishInitial(t)
		publishedAt := s.now
		username := s.rotation.Spec.Usernames[0]
		...
		s.expectService()
		s.api.EXPECT().
			ServiceUserCreate(mock.Anything, s.rotation.Spec.Project, s.rotation.Spec.ServiceName, &service.ServiceUserCreateIn{Username: username}).
			Return(&service.ServiceUserCreateOut{Username: username, Password: "new-api-password"}, nil).Once()
		s.expectCA()

		_, err := s.reconcile(t)
		require.NoError(t, err)
		s.requirePublished(t, username, "generated-1", publishedAt)
		require.Equal(t, 1, s.generated)
	})
	...
	for _, missing := range []string{"user", "certificate", "key"} {
		t.Run("Retries an incomplete Kafka response/"+missing, func(t *testing.T) {
			s := newRotationScenario(t)
			s.useKafka()
			s.expectService()
			users := slices.Clone(s.service.Users)
			...
			case "user":
				users = users[1:]
			case "certificate":
				users[0].AccessCert = nil
			case "key":
				users[0].AccessKey = new("")
			}
			username := s.rotation.Spec.Usernames[0]
			s.api.EXPECT().
	...
	for _, publication := range []string{"initial", "scheduled", "retry"} {
		t.Run("Starts the rotation interval after a slow CA request/"+publication, func(t *testing.T) {
			...
			publishedAt := s.now.Add(2 * time.Minute)
			s.expectService()
			s.expectReset(username, password)
			...
			s.now = publishedAt.Add(time.Minute - time.Second)
			s.expectService()
			s.expectCA()
			res, err = s.reconcile(t)
			require.NoError(t, err)
	...
	t.Run("Computes requeue after publication", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			{"before deadline", time.Second, 2 * interval, interval - time.Second},
			{"at deadline", interval, 2 * interval, time.Nanosecond},
			{"past deadline", interval + time.Second, 2 * interval, time.Nanosecond},
			{"poll cap", time.Second, time.Minute, time.Minute},
			{"default poll", time.Second, 0, DefaultPollInterval},
			{"negative poll", time.Second, -time.Minute, DefaultPollInterval},
		} {
			t.Run(tc.name, func(t *testing.T) {
				...
				res, err := s.reconcile(t)
				require.NoError(t, err)
				require.Equal(t, tc.want, res.RequeueAfter)
				s.requirePublished(t, s.rotation.Spec.Usernames[0], "generated-1", publishedAt)
			})
		}
	})
	...
	for _, problem := range []string{"not found", "API failure", "power off", "not ready"} {
		t.Run("Reports a stable error until the service becomes operational/"+problem, func(t *testing.T) {
			s := newRotationScenario(t)
			var cause error
			switch problem {
			case "not found":
				cause = newAivenError(404, "service missing")
			case "API failure":
				cause = errors.New("service request failed")
			case "power off":
				s.service.State = service.ServiceStateTypePoweroff
			case "not ready":
			...
			for range 2 {
				...
				if previousVersion != "" {
					require.Equal(t, previousVersion, s.object(t).ResourceVersion)
				}
				previousVersion = s.object(t).ResourceVersion
				require.Zero(t, s.generated)
			}
	...
	t.Run("Keeps users created before an interrupted pool initialization", func(t *testing.T) {
		s := newRotationScenario(t)
		users := slices.Clone(s.service.Users)
		s.service.Users = nil
		failure := errors.New("second user creation failed")
		s.expectService()
		s.api.EXPECT().
			ServiceUserCreate(mock.Anything, s.rotation.Spec.Project, s.rotation.Spec.ServiceName, &service.ServiceUserCreateIn{Username: users[0].Username}).
			Return(new(service.ServiceUserCreateOut(users[0])), nil).Once()
		s.api.EXPECT().
			ServiceUserCreate(mock.Anything, s.rotation.Spec.Project, s.rotation.Spec.ServiceName, &service.ServiceUserCreateIn{Username: users[1].Username}).
			Return(nil, failure).Once()
	...
	for _, deleting := range []bool{false, true} {
		for _, problem := range []string{"auth Secret missing", "client initialization"} {
			t.Run(fmt.Sprintf("Reports authentication errors/%s/deleting=%t", problem, deleting), func(t *testing.T) {
				s := newRotationScenario(t)
				if deleting {
					s.startDeletion(t)
				}
				failure := errors.New("client initialization failed")
				if problem == "auth Secret missing" {
	...
	for _, target := range []string{"auth Secret", "rotation"} {
		t.Run("Stops before API requests when a finalizer cannot be saved/"+target, func(t *testing.T) {
			s := newRotationScenario(t)
			key := crclient.ObjectKeyFromObject(s.rotation)
			if target == "auth Secret" {
				key = crclient.ObjectKeyFromObject(s.auth)
			}
			failure := errors.New("finalizer write failed")
			s.r.Client = interceptor.NewClient(s.client, interceptor.Funcs{
				Update: func(ctx context.Context, c crclient.WithWatch, obj crclient.Object, opts ...crclient.UpdateOption) error {
					if crclient.ObjectKeyFromObject(obj) == key {
						return failure
					}
					return c.Update(ctx, obj, opts...)
				},
	...
	t.Run("Deletion", func(t *testing.T) {
		t.Run("Deletes every user, accepts NotFound, and does not save status after finalization", func(t *testing.T) {
			s := newRotationScenario(t)
			s.startDeletion(t)
			for i, username := range s.rotation.Spec.Usernames {
				var deleteErr error
				if i == 0 {
					deleteErr = newAivenError(404, "already deleted")
				}
				s.api.EXPECT().ServiceUserDelete(mock.Anything, s.rotation.Spec.Project, s.rotation.Spec.ServiceName, username).
			...
			s.r.Client = interceptor.NewClient(s.client, interceptor.Funcs{
				SubResourceUpdate: func(_ context.Context, _ crclient.Client, _ string, _ crclient.Object, _ ...crclient.SubResourceUpdateOption) error {
					t.Fatal("successful deletion must not write status")
					return nil
				},
			})
			res, err := s.reconcile(t)
		...
		t.Run("Orphan finalizes without an auth Secret or API requests", func(t *testing.T) {
			s := newRotationScenario(t)
			s.configure(t, func(cr *v1alpha1.ServiceUserRotation) {
				cr.Annotations = map[string]string{deletionPolicyAnnotation: deletionPolicyOrphan}
			})
			s.startDeletion(t)
			require.NoError(t, s.client.Delete(t.Context(), s.auth))
			res, err := s.reconcile(t)
		...
		for _, code := range []int{403, 500} {
			t.Run(fmt.Sprintf("Retries all users after a partial failure/%d", code), func(t *testing.T) {
				s := newRotationScenario(t)
				s.startDeletion(t)
				failure := newAivenError(code, "user deletion failed")
				for i, username := range s.rotation.Spec.Usernames {
					var deleteErr error
					if i == 0 {
						deleteErr = failure
				...
				for _, username := range s.rotation.Spec.Usernames {
					s.api.EXPECT().ServiceUserDelete(mock.Anything, s.rotation.Spec.Project, s.rotation.Spec.ServiceName, username).
						Return(newAivenError(404, "already deleted")).Once()
				}
				_, err = s.reconcile(t)
				require.NoError(t, err)
				require.True(t, apierrors.IsNotFound(s.client.Get(t.Context(), crclient.ObjectKeyFromObject(s.rotation), &v1alpha1.ServiceUserRotation{})))
		...
		t.Run("Preserves multiple deletion errors and a status error", func(t *testing.T) {
			s := newRotationScenario(t)
			s.startDeletion(t)
			firstErr, secondErr := errors.New("first user failed"), errors.New("second user failed")
			...
			writes := 0
			s.r.Client = interceptor.NewClient(s.client, interceptor.Funcs{
				SubResourceUpdate: func(_ context.Context, _ crclient.Client, _ string, obj crclient.Object, _ ...crclient.SubResourceUpdateOption) error {
					writes++
					cr := obj.(*v1alpha1.ServiceUserRotation)
					condition := meta.FindStatusCondition(cr.Status.Conditions, ConditionTypeError)
					require.NotNil(t, condition)
					require.Contains(t, condition.Message, firstErr.Error())
					require.Contains(t, condition.Message, secondErr.Error())
					require.NotContains(t, condition.Message, statusErr.Error())
					return statusErr
				},
			})
			res, err := s.reconcile(t)
			...
			require.Equal(t, 1, writes)
			require.Contains(t, s.object(t).Finalizers, instanceDeletionFinalizer)
			require.Len(t, recorderEvents(s.recorder), 1)
		})

		t.Run("Reports an invalid deletion policy", func(t *testing.T) {
			s := newRotationScenario(t)
			s.configure(t, func(cr *v1alpha1.ServiceUserRotation) {
				cr.Annotations = map[string]string{deletionPolicyAnnotation: "invalid"}
			})
			s.startDeletion(t)
			res, err := s.reconcile(t)
			require.ErrorContains(t, err, "invalid deletion policy")
			s.requireFailure(t, res, err)
			require.Contains(t, s.object(t).Finalizers, instanceDeletionFinalizer)
		})

		t.Run("Reports and retries a failed finalizer removal", func(t *testing.T) {
			s := newRotationScenario(t)
			s.configure(t, func(cr *v1alpha1.ServiceUserRotation) {
				cr.Annotations = map[string]string{deletionPolicyAnnotation: deletionPolicyOrphan}
			})
			s.startDeletion(t)
			failure := errors.New("finalizer removal failed")
			s.r.Client = interceptor.NewClient(s.client, interceptor.Funcs{
				Update: func(ctx context.Context, c crclient.WithWatch, obj crclient.Object, opts ...crclient.UpdateOption) error {
					if cr, ok := obj.(*v1alpha1.ServiceUserRotation); ok && !slices.Contains(cr.Finalizers, instanceDeletionFinalizer) {
						return failure
					}
					return c.Update(ctx, obj, opts...)
				},
...
func (s *rotationScenario) startDeletion(t *testing.T) {
	t.Helper()
	cr := s.object(t)
	if !slices.Contains(cr.Finalizers, instanceDeletionFinalizer) {
		cr.Finalizers = append(cr.Finalizers, instanceDeletionFinalizer)
		require.NoError(t, s.client.Update(t.Context(), cr))
	}
	require.NoError(t, s.client.Delete(t.Context(), cr))
}
