...
func (r *ServiceUserRotationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	// Jitter periodic polls without postponing the rotation deadline.
	return ctrl.Result{RequeueAfter: min(
		wait.Jitter(r.pollInterval(), pollJitterFactor),
		max(time.Nanosecond, cr.Status.NextRotationAt.Sub(r.now())),
	)}, nil
}
