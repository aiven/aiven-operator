...
func (c *SecretWatchController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := c.Get(ctx, req.NamespacedName, secret); err != nil {
		if errors.IsNotFound(err) {
			c.Log.Info("secret not found, nothing to reconcile", "secret", req.NamespacedName)
...
func (c *SecretWatchController) triggerReconciliation(ctx context.Context, resource SecretSourceResource) error {
	...
	annotations["controllers.aiven.io/secret-source-updated"] = fmt.Sprintf("%d", time.Now().Unix())

	// clear the processed generation annotation to force the basic controller to reconcile
	delete(annotations, "controllers.aiven.io/generation-was-processed")
	...
	c.Log.Info("triggered reconciliation for resource",
		"resource", types.NamespacedName{Name: latest.GetName(), Namespace: latest.GetNamespace()},
